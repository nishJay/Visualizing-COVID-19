...
# Read the dataset using the provided column specification
...
# Label the y-axis
# Create a line plot
p <- ggplot(confirmed_cases_worldwide, aes(x = date, y = cum_cases)) +
  geom_line() +
...
plt_cum_confirmed_cases_china_vs_world <- ggplot(confirmed_cases_china_vs_world) +
  ...
  labs(title = "Cumulative Confirmed Cases in China vs Rest of the World",
       x = "Date",
       y = "Cumulative confirmed cases") +
  theme_minimal()
...
plt_cum_confirmed_cases_china_vs_world <- ggplot(confirmed_cases_china_vs_world, aes(x = date, y = cum_cases, color = is_china)) +
  ...
  geom_text(data = who_events, aes(x = date, y = 100000, label = event), vjust = -0.5, color = "red", angle = 90) +
  labs(title = "Cumulative Confirmed Cases in China vs Rest of the World",
       x = "Date",
       y = "Cumulative confirmed cases") +
  theme_minimal()
...
china_after_feb15 <- confirmed_cases_china_vs_world %>% filter(is_china == "China", date >= as.Date("2020-02-15"))

# Using china_after_feb15, draw a line plot cum_cases vs. date
plt_china_after_feb15 <- ggplot(china_after_feb15, aes(x=date,y=cum_cases))+geom_line()+geom_smooth(method = "lm", se = FALSE, color = "red")+labs(title = "Cumulatie Confirmed Cases in China(After Feb 15,2020)",x= "Date", y ="Cumulative confirmed cases")+theme_minimal()
...
plt_not_china_trend_lin <- ggplot(not_china, aes(x = date, y = cum_cases)) +
  geom_line() + geom_smooth(method = "lm", se = FALSE, color = "red")+
  labs(title = "Cumulative Confirmed Cases in Rest of the World",
       x = "Date",
       y = "Cumulative confirmed cases") +
  theme_minimal()
...
plt_not_china_trend_lin + scale_y_log10()+labs(y="Logarithmic Cumulative confirmed cases")
```
...
grouped_by_country <- confirmed_cases_by_country %>%
  group_by(country)
summary_by_country <- grouped_by_country %>%
  summarize(total_cases = max(cum_cases))
top_countries_by_total_cases <- summary_by_country %>%
...
plt_cum_cases_by_country <- ggplot(confirmed_cases_top7_outside_china, aes(x = date, y = cum_cases, color = country)) +
  geom_line() +
  labs(title = "Cumulative Confirmed Cases in Top 7 Countries outside China",
       x = "Date",
       y = "Cumulative confirmed cases") +
  theme_minimal()
plt_cum_cases_by_country
```
